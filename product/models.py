...
class Product(models.Model):
    ...
    cost_price = models.DecimalField(max_digits=10, null=True,
                                     decimal_places=2,
                                     validators=[MinValueValidator(0)])
    ...
    def profit_margin(self):
        """
        Calculates the profit margin of
        a product: {{ product.profit_margin }}
        """
        if self.discounted:
            margin = self.discounted_price - self.cost_price
            profit_margin_percent = (margin / self.cost_price) * 100
            return profit_margin_percent.quantize(
                Decimal('0.01'), rounding=ROUND_DOWN)
    ...
    def profit_amount(self):
        """
        Calculates the amount of profit
        generated per product {{ product.profit_amount }}
        """
        if self.discounted:
            margin = (
                self.discounted_price - self.cost_price) / self.cost_price
    ...
    def clean(self):
        ...
        if self.quantity_available < 0:
            raise ValidationError(
                    {'quantity_available': 'Quantity must be non-negative.'})
...
class Size(models.Model):
    ...
    product = models.ForeignKey("Product", on_delete=models.CASCADE)
    alternate_size = models.CharField(
        max_length=10, choices=SIZE_CHOICES, null=True,
        blank=True, default=None)
    size_quantity_available = models.PositiveIntegerField(
